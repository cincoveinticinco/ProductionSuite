...
(CASE INSTR(numero_escena, '.') 
	...
	public function nomina_personajes_mensuales($idproduccion,$fecha1,$fecha2){
		$query = $this->db->query("SELECT elemento.id AS idelemento, elemento.nombre, UPPER(rol_actores_elementos.rol) AS rol, 
			UPPER(CASE WHEN elemento.actor_nombre IS NULL THEN ' - ' ELSE elemento.actor_nombre END) AS actor_nombre, 
			UPPER(CASE WHEN elemento.actor_apellido IS NULL THEN ' - ' ELSE elemento.actor_apellido END) AS actor_apellido, 
			UPPER(CASE WHEN tipo_contrato.descripcion IS NULL THEN ' - ' ELSE tipo_contrato.descripcion END) AS contrato,
	...
	public function descuento_personaje($id_elemento,$mes){
		$query = $this->db->query("SELECT descuentos_nomina.descuento,descuentos_nomina.observaciones FROM descuentos_nomina
						INNER JOIN liquidaciones ON descuentos_nomina.id_liquidacion = liquidaciones.id
	...
	/*public function buscar_capitulos_elemento($id_elemento,$fecha_inicio,$fecha_final){
		$query = $this->db->query("SELECT group_concat(DISTINCT(produccion_has_capitulos.numero),' ', 
									(SELECT DATE_FORMAT(MIN(e2.fecha_produccion), '%d-%b-%Y') FROM escena e2 WHERE e2.id_capitulo = produccion_has_capitulos.id AND e2.fecha_produccion !='0000-00-00')
									...
									AND produccion_has_capitulos.id NOT IN (
	...
	public function update_escenas_has_elemtento($id_personaje1,$id_personaje2){
		...
		$this->db->update('escenas_has_elementos',$datos);
	}
