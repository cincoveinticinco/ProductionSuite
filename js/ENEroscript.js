 ...
 // $('.liquidacion').click(function(){
    ...
    $.ajax({
      ...
      success:function(data){
        if(data.user){
          ...
          user_final='<tr class="sort_left"><td colspan="4">No hay usuarios en esta categoría</td></tr>'
        }
  ...
  $('.main').on('click','.save_icon',function(){
    ...
    $.ajax({
      ...
      dataType: "json",
      success:function(data){
        if(opcion!="fecha_aire" && opcion!="numero_escenas"){
          ...
          document.getElementById(opcion+"_text"+idproduccion+'_'+idcapitulo).innerHTML = valor.substring(0,15)+"";
            ...
            if(data.validacion){
              ...
              alert("No puede cambiarse la cantidad de escenas porque las escenas desglosadas son mayor a las ingresadas");
            } 
        }
      }
    });
  ...
  $('#wrapper').on('click','.cancel_cap',function(){
    var idproduccion = $(this).attr('idproduccion');
    var idcapitulo = $(this).attr('idcapitulo');
    var confirmacion = "¿Deséa cancelar el libreto y eliminar ("+$(this).attr('data-noproducudos')+") escenas no producidas";
    if($(this).attr('planes-producidos') !=""){
      confirmacion+= ", y cancelar ("+$(this).attr('planes-producidos')+") escenas producidas";
    }
    if($(this).attr('planes-asignados')!=""){
      confirmacion += " y las asignadas el " + $(this).attr('planes-asignados');
    }
    confirmacion+=" ?";
  ...
  $('.delete_cap').click(function(){
    var idproduccion = $(this).attr('idproduccion');
    var idcapitulo = $(this).attr('idcapitulo');
    var confirmacion = "¿Está seguro de que desea eliminar el libreto?";
    if($(this).attr('planes-asignados')!=""){
      confirmacion = "¿Está seguro de que desea eliminar el libreto (incluyendo las escenas asignadas a los planes de produccion: " + $(this).attr('planes-asignados') + " )?";
    }
    if(confirm(confirmacion)){
      datos= {idproduccion: idproduccion, idcapitulo: idcapitulo}
      $.ajax({
  ...
  $('.action_escena').click(function() {
    ...
    if(opcion!="duplicar"){
      ...
      var confirmacion = "¿Está seguro que desea "+opcion+"?";
      if(opcion=="eliminar" && $(this).attr('asignacion')!='-'){
        confirmacion = "¿Está seguro que desea desasignar la escena del plan "+$(this).attr('asignacion')+" y eliminarla?";
      }
      if(confirm(confirmacion)){
        $('#wrapper_loader').fadeIn();
        $.ajax({
      ...
      datos2= {idcapitulo: idcapitulo,numero_escena:numero_escena}
        $.ajax({
          ...
          success:function(data){
            if(data.existe==true){
                datos= {idproduccion: idproduccion, idcapitulo: idcapitulo,opcion:opcion,idescena:idescena,numero_escena:numero_escena}
                if(confirm("¿Está seguro que desea "+opcion+"?")){
                  $('#wrapper_loader').fadeIn();
                  $.ajax({
                    type: "POST",
                    url: site_url+"/escenas/acciones_escenas",
               ...
               $('#numero_escena').addClass('error');
                html1 = '<label class="error label-error-hiden">Esta escena ya existe.</label>';
  ...
  $('#location').change( function(){
      ...
      success:function(data){
          ...
          if(data.set){
            ...
            if(pathname.search("plan_general/")>0){
              ...
              sets = sets+"<option value=''>no hay set para esta locacion</option>";
            }
          }
          $("#set").css('background','');
         $("#set").html(sets);
    ...
    $('#locacion_select_cambio').change( function(){
  ...
  $('#add_item_location').click(function(){
    ...
    if(required1 == '' || required2 == '' ){
        if (counter_add_item_location != 1) {
          ...
          html1 = '<label class="error label-error-hiden">Este campo es requerido.</label>';
          $('#new_location').parent().append(html1);

          $('#new_set').addClass('error');
          html2 = '<label class="error label-error-hiden">Este campo es requerido.</label>';
      ...
      $.ajax({
        ...
        if(!data.respuesta){
          $.ajax({
            ...
            success:function(data){
               if(data.locacion){
                  ...
                  opciones = opciones+"<option value='null'>no hay set para esta locacion</option>";
                }
                  var set="<option value=''>no hay set para esta locacion</option>";
                  $('#add_item_location').fadeOut();
                  $('#new_location').fadeOut();
                  $('#new_location').val('');
                  $('#new_item_location').fadeIn();
                  $("#location").html(opciones).fadeIn();
                  $("#set").html(set);
          ...
          $('#new_set').val(temp);
          alert("Ya existe una locación con este nombre");
        };
        }
      });
      }
   ...
   $('#add_item_set').click(function(){
      ...
      if(required == ''){
        ...
        $.ajax({
          ...
          var temp = $('#new_set').val();
          $('#new_set').val(temp);
          alert("Ya existe un set con este nombre para esta locación");
        };
        }
      });
    }
  ...
  $('#create_element_button').click(function(){
    ...
    $.ajax({
      ...
      success:function(data){
        if(!data.respuesta){
          $('#element_name').parent().find('label.error').html('Este elemento ya existe en esta categoria');
        }
  ...
  var counter_crear_elemento = '';
   $('#crear_elemento').click(function(){
    ...
    if (confirm('Desea crear este elemento '+required)) { 
                ...
                if(required == ''){
                  if (counter_crear_elemento != 1) {
                    ...
                    html = '<label class="error label-error-hiden">Este campo es requerido.</label>';
                  ...
                  $.ajax({
                    ...
                    success:function(data){
                      if(data.respuesta==1){
                        alert('Este elemento ya existe en esta categoria');
                        return false;
                      }else{
                        $('#name_elemento').parent().find('.alert-box').remove();
                        $('#name_elemento').parent().append('<div class="alert-box success">Elemento creado con éxito<a href="" class="close" tabindex="-1">×</a></div>');
                        $('#name_elemento').val("");
      ...
      success:function(data){
        if(data.elementos){
         $.each(data.elementos, function(i,elementos){
          ...
          opciones=opciones+"<tr class='ui-drag-disabled'> class='sort_left'><td>No hay elemento para esta categoria</td></tr>";
        }
          ...
          $.ajax({
            ...
            success:function(data){
              if(data.elementos){
                ...
                opciones=opciones+"<tr class='ui-drag-disabled'> class='sort_left'><td>No hay elementos para la busqueda</td></tr>";
              }
              $("#tabla1").html(opciones);
            }
          })
  ...
  $('#tabla1').on('click', '.agregar_elemento', function(){
    ...
    var opciones= '';
    var opciones=opciones+'<tr class="sort_left">';
    opciones=opciones+'<td width="30%"><input type="checkbox" name="elemento['+id_elemento+']" value='+id_elemento+' style="display:none;" checked><input type="hidden" value='+id_elemento+' id="id_elemento_escenas"><input type="hidden" value="'+tipo+'" id="tipo"><input type="hidden" value="'+nombre+'" id="nombre"><span>'+tipo+'</span></td>';
    opciones=opciones+'<td width="50%">'+nombre+' '+rol;
        if(rol.indexOf('( Extra )') >0 && pathname.search("escena")>0 ){ 
         ...
         console.log(tipo.search('background'))
      ...
      $.ajax({
        ...
        success:function(data){
          if(data.eliminar==1){
            ...
            alert('Esta categoria no se puede eliminar ya que sus elementos se estan usando en una escena');
          }
        }
      }); 
    }
  ...
  $("#produccion_estado").click(function(){
    if(confirm("¿Quieres cambiar el estado de esta produccion?")) {
  ...
  function comprueba_extension() { 
    ...
    if (archivo!=""){
      ...
      if (!permitida) {
        ...
        $('#image_production').parent('div').append('<label class="error2">El formato del archivo no es valido</label>');
    ...
    if(response){
      if(parseInt($('#segundos_capitulo').val())>59){
        ...
        $('#segundos_capitulo').parent('div').append('<label class="error2">El valor debe ser menor a 60</label>');
    ...
    if(response){
      if(parseInt($('#seg_proy_sem').val())>59){
        ...
        $('#seg_proy_sem').parent('div').append('<label class="error2">El valor debe ser menor a 60</label>');
    ...
    if(response){
        $(".maxnumber").each(function(){
            ...
            if(parseInt(valor)>999){
              response = false;
              $(this).addClass('error');
              $(this).parent('div').append('<label class="error2">El valor maximo es 999</label>');
    ...
    if(response && pathname.indexOf('produccion/index')>0){
      if($('#date_online').val()!=""){
        if($('#lunes').is(':checked') || $('#martes').is(':checked') || $('#miercoles').is(':checked')  || $('#jueves').is(':checked') || $('#viernes').is(':checked') || $('#sabado').is(':checked') || $('#domingo').is(':checked')){
          ...
          $('#online_days').append('<label class="error2">Seleccione al menos 1 día</label>');
     ...
     $('.table_general').on('click','.save_script',function(){
        ...
        if (archivo!=""){
          ...
          if (!permitida) {
            ...
            $('#libreto'+idcapitulo).parent('div').append('<label class="error2">El formato del archivo no es valido</label>');
          ...
          success:function(data){
            if(data.cadena!=false && data.cadena!=null){
              ...
              $.ajaxSetup({ cache: true });
                $.getScript(site_url+"/js/app.js", function(){
...
$(document).ready(function() {
    $('.no_scene').click(function(){
      alert("Aún no ha ingresado el número de escenas para el libreto "+ $(this).attr('data-numero'));
    });
    ...
    $('.borrarSeleccionadas').click( function(){
      if(confirm("¿Esta seguro que desea eliminar estas escenas?")){
        ...
        alert("Escenas eliminadas exitosamente");
        //window.location.reload();
      }
    });
...
/////fin crear categoria elementos/////
 ...
 $('#enviar_form_mover_plan').click(function(){
    ...
    if(!fecha){
      $('#wrapper_loader').hide();
      $('#fecha_unidad_mover').parent('div').append('<label class="error" style="color:red!important;">Debe ingresar fecha</label>');
      return false;
    }else{
       var unidad=$('#unidad_plan_mover option:selected').val();
       if(!unidad){
           $('#wrapper_loader').hide();
          $('#unidad_plan_mover').parent('div').append('<label class="error error_unidad" style="color:red!important;">Debe Seleccionar una unidad</label>');
          return false;
       }else{
...
function validation_duration(){
  var pathname = window.location.pathname;
  if(confirm(' Esta seguro que quiere guardar la escena?')){
        ...
        if((duration_minutes=="" || duration_minutes=="00" || duration_minutes=="0")  && (duration_seconds=="" || duration_seconds=="00" || duration_seconds=="0")
          ...
          $('#duration_minutes').parent('div').append('<label class="error2">Debe ingresar los minutos o segundos</label>');
          ...
          if(duration_seconds<60){
            ...
            $('#duration_seconds').addClass('error'); 
            $('#duration_seconds').parent('div').append('<label class="error2">los segundos deben ser menores a 60</label>');
          ...
          numero_escena =  $('#numero_escena').val();
            datos2= {idcapitulo: idcapitulo,numero_escena:numero_escena}
              $.ajax({
                ...
                success:function(data){
                  if(!data.existe){
                    tempo.append('<label class="error2" style="font-size: 11px; text-transform: inherit; margin-bottom: 10px; font-weight: normal; clear: both;color: #c60f13;">Ya existe una escena con este número</label>');
                    $('#numero_escena').focus();
                    retorno = false;
                  }else{
                    retorno = true;
...
function validation_fecha(){
  ...
  if(!fecha){
    $('#wrapper_loader').hide();
    $('#fecha_unidad_plan').parent('div').append('<label class="error" style="color:red!important;">Debe ingresar fecha</label>');
    return false;
  }
}
...
$(document).ready(function() {
  ...
  if(pathname.search("plan_general/index")>0 || pathname.search("plan_general/filtro")>0){
    ...
    $('#unidad_selector').change(function(){
      ...
      var html2 = '<label class="error label-error-hiden">Esta unidad no tiene fecha asignada</label>';
      if(minDate!=""){
    ...
    $('#insert_asing_button').click(function(){
      ...
      if(pathname.search("plan_general/")>0){
        ...
        if( (Date.parse($('#start_plan').val())/ 1000) < (Date.parse($('#date_unity'+$('#unidad_selector').val()).html())/ 1000) ){
          var html = '<label class="error label-error-hiden">Fecha invalida</label>';
          ...
          if(estado){
             $.each(ids, function(j,ids){             
                ...
                $('#row2_'+ids).attr('data-unidad') != $('#unidad_selector').val()){
                        ...
                        if(data.resultado){
                          $.each(data.plan, function(i,plan2){
                            if(plan2.id_escena ==ids ){
                              console.log(plan2);
                              confirmacion += 'La escena '+plan2.numero_capitulo+'/'+ plan2.numero_escena + ' que se ha asignado al plan del día ' +plan2.fecha_inicio_f+' U'+ plan2.numero_unidad +', ';
                          ...
                          if(pathname.search("plan_diario/")>=0){
                            confirmacion += 'La escena ' + $( "#capitulos option:selected" ).text() + '/' + $( "#escenas option:selected" ).text()+', ';
                          }else{
                            confirmacion += 'La escena ' + $('#row2_'+ids).attr('data-libreto') + '/' +$('#row2_'+ids).attr('data-numero')+', ';
                          }
                  ...
                  alert('La escena '+ $('#row2_'+ids).attr('data-libreto')+ '/' +$('#row2_'+ids).attr('data-numero')+' ya pertenece a este plan');
                }
                }  
              });
            ...
            if(pathname.search("plan_diario/")>=0){
              ...
              if(!confirm('¿Deseá asignar '+confirmacion+' al plan diario del día '+$('#fecha_unidad_plan').val()+' de la unidad '+unidad_selected.replace("unidad","")+' ?')){
              ...
              if(!confirm(confirmacion+' ¿Deseá asignar estas escenas al plan diario del día '+$('#start_plan').val()+' de la unidad '+unidad_selected.replace("unidad","")+' ?')){
                var temp_e = comparacion.split(',');
                for (var i = 0; i < temp_e.length; i++) {
                   $('#idescenas').val($('#idescenas').val().replace(temp_e[i]+',', ""));
                };
            ...
            $('#wrapper_loader').fadeOut();
            alert('Este plan esta cerrado');
          }
        }else{
          var html = '<label class="error label-error-hiden">Este campo es requerido</label>';
              ...
              var m=0;
                if(escena.estado!=3 && data.usuario_permisos=="write"){
                          ...
                          if ((escena.estado == 1 &&  escena.fecha_produccion!=null &&  escena.fecha_produccion!="0000-00-00") || (escena.estado == 2 &&  escena.fecha_produccion!=null &&  escena.fecha_produccion!="0000-00-00") ) {
                              cadena_tabla+= '<a href="'+site_url+'/plan_diario/index/'+data.produccion[0].id_produccion+'/'+escena.unidad_produccion+'/'+escena.fecha_produccion+'">'+escena.fecha_produccion_2+'</a>';
                          } else {
                            if (escena.fecha_inicio != null && escena.fecha_inicio != "0000-00-00") {
                              cadena_tabla+= '<a href="'+site_url+'/plan_diario/index/'+data.produccion[0].id_produccion+'/'+escena.idunidad+'/'+escena.fecha_inicio+'">'+escena.fecha_inicio+'</a>';
                            } else {
                              cadena_tabla+= '-';
                            }
                          }
...
if(data.usuario_permisos=="write"){
  ...
  if(validate_click=='0'){
    ...
    $('.tabla_filtro_general').append(cadena_tabla);
          ...
          $('footer').append("<style> .tooltip { word-break: break-all; }</style><script>  $('.oldPlan').on('dblclick',function(){alert('Esta escena esta asignada al plan del '+$(this).attr('data-plan')+', que aún esta abierto');});</script>");
        ...
        $('#alert_filter').fadeOut();
            ...
            $('#consultaExcel').val(data.consultaImpresion);
                if(escena.estado!=3 && data.usuario_permisos=="write"){
                          ...
                          if (escena.estado == 1 &&  escena.fecha_produccion!=null &&  escena.fecha_produccion!="0000-00-00") {
                              cadena_tabla+= '<a href="'+site_url+'plan_diario/index/'+data.produccion[0].id_produccion+'/'+escena.unidad_produccion+'/'+escena.fecha_produccion+'">'+escena.fecha_produccion+'</a>';
                          } else {
                            if (escena.fecha_inicio != null && escena.fecha_inicio != "0000-00-00") {
                              cadena_tabla+= '<a href="'+site_url+'plan_diario/index/'+data.produccion[0].id_produccion+'/'+escena.idunidad+'/'+escena.fecha_inicio+'">'+escena.fecha_inicio+'</a>';
                            } else {
...
/*FIN FUNCION PARA FECHAS PLAN PRODUCCION*/
    ...
    function addclass_error(){
      ...
      $('#name_production').parent('div').append('<label class="error2">Nombre de producción ya asignado</label>');
    ...
    $(".delete_unity").click(function(){
      ...
      $.ajax({
        ...
        dataType: "json",
          success:function(data){
            if(data.asignados == '1'){
              alert("Esta unidad no puede eliminarce ya que posee planes diarios asignados");
            }else{
  ...
  $('.delete_scene_plan').click(function(){
    ...
    if(confirm("¿Está seguro que desea eliminar la escena " + $(this).attr('data-numerolibreto') + "/"+ $(this).attr('data-numeroescena') +" de este plan?")){
      $('#wrapper_loader').fadeIn();
 ...
 $('#find_plan').click(function(){
  ...
  $('#unidad_plan').change(function(){
    if($('#unidad_plan').val()!=""){
      ...
      $('label.error').remove();
      var minDate = $("#date_unity" + $(this).val()).html();
      var html2 = '<label style="color: #c60f13!important;" class="error label-error-hiden">Esta unidad no tiene fecha asignada</label>';
      if(minDate!=""){
  ...
  function confirm_link(){
    if(pathname.search("crear_escena")>0 || pathname.search("editar_escena")>0 || pathname.search("/produccion/index")>0){
      if(confirm("¿Está seguro que desea salir? todos los datos se perderan")){
        return true;
      }else{
        return false;
      }
  ...
  $('.oldPlan').on("dblclick",function(){
    alert('Esta escena esta asignada al plan del '+$(this).attr('data-plan')+', que aún esta abierto');
  });
      ...
      var rol ="";
        if(palabra){
          ...
          $.each(data.usuarios, function(i,usuarios){
            ...
            opciones=opciones+"<tr class='ui-drag-disabled'> class='sort_left'><td>No hay usuarios para la busqueda</td></tr>";
          }
...
$('#semanal_pdf').click(function(){
  ...
  if(fecha!=""){
    ...
    if(checked){
      ...
      $("#semanal_pdf").parent().append("<label class='error'>Selecciona una unidad</label>");
    }
  }
});
...
$('#semanal_element_pdf').click(function(){
  ...
  if(fecha!="" && unidades!="" && opcion!=""){
    ...
    $('#semanal_elementos').append('<label class="error">Seleccione una opción de reporte</label>');
  }
});
...
$('#nomina_pdf').click(function(){
  ...
  var html= "<label class='error'>Rango de fechas Invalido</label>"
        ...
        success:function(data){
          if(data.resultado){
            ...
            if(data.cadena_liquidacion!=""){
              ...
              $('#discounts_actors_section').html("<label>No hay personajes mensuales asignados a esta semana</label>");
            }
          }else{
...
$('#pdf_plan_general').click(function(){
  ...
  if(total_escenas>300){
    alert('Se imprimirá un maximo de 300 escenas en este informe');  
  }
...
$('.save_tools').click(function(){
  ...
  var valida = true;
  var html= "<label class='error'>Campo requerido</label>"
  var alerta="";
  $('#tipo').val(tipo);
  ...
  if(tipo=='1'){
    ...
    if(dia_continuidad=="" || dia_continuidad_actual==""){
      ...
      if(confirm("Esta seguro que desea cambiar la continuidad de las escena(s) desde "+from+" hasta "+ to)){
        valida= true;
        alerta = "el día de continuidad " + $('#dia_continuidad').val();
      }else{
        valida= false;
      }
    }
    ...
    var set = $('#set').val();
    var location_actual = $('#locacion_select_cambio').val();
    ...
    if(set == null || set == "" || location == "Seleccione una locación"){
      ...
      if(confirm("Esta seguro que desea cambiar la locacion y/o set de las escena(s) desde "+from+" hasta "+ to)){
        valida= true;
        alerta = "la locacion " + $("#location option:selected").html().toUpperCase() + " y el set " + $("#set option:selected").html().toUpperCase();
      }else{
        valida= false;
      }
    }
    ...
    /*}else if($('.locacion_tipo_cambiar:checked').size()==0){
      ...
      if(confirm("Esta seguro que desea cambiar la locacion / estudio de las escena(s) desde "+from+" hasta "+ to)){
        valida= true;
   ...
   // var dia_noche_cambiar = $('.dia_noche_cambiar:checked').val();
    ...
    /*}else if($('.dia_noche_cambiar:checked').size()==0){
      ...
      if(confirm("Esta seguro que desea cambiar  día / noche de las escena(s) desde "+from+" hasta "+ to)){
        valida= true;
    ...
    /*}else if($('.int_ext_cambiar:checked').size()==0){
      ...
      if(confirm("Esta seguro que desea cambiar  Interior / esterior de las escena(s) desde "+from+" hasta "+ to)){
        valida= true;
  ...
  if(valida){
    if($('#herramientas_form').valid()){
      ...
      datos= {
        ...
        location_actual:$('#locacion_select_cambio').val(),
        set_actual:id_set_actual,
        location_cambio:$('#location').val(),
        set_cambio:$('#set').val(),
...
$('.unificacion_personaje').click(function(){
  ...
  console.log(personaje1.search(personaje2));

  if(personaje1.search(personaje2)<0){

    if(confirm("Esta seguro de Unificar el actor "+personaje1.toUpperCase()+" al actor "+personaje2.toUpperCase()+". Esto eliminaria al Actor "+personaje1.toUpperCase()+"?")){
                $('#wrapper_loader').fadeIn();
...
$('#usersTable').on('click','.cambiar_user',function(){
  ...
  $('#usersTable').on('change','.rol_personaje',function(){
     ...
     if(val=="Extra"){
      alert('Si este personaje tiene escenas con multiples extras asociados a este rol cambiara a uno');
     }
  });
  ...
  $('#wrapper').on('change','.tamsa_cambio',function(){
         ...
         if(isNaN(valor)){
            alert('Se debe ingresar un valor valido');
            $(this).val('')
         }else{
              valor=valor*moton;
   ...
   $('.eliminar_fotos').click(function(){
       ...
       if(confirm('Esta seguro de eliminar todas las fotos de esta produccion? Recuerde que si las elimina no se podran recuperar')){
         $('#wrapper_loader').fadeIn();
...
function calculo_tiempo_post(minutos1,segundos1,cuadros1){
     ...
     function validation_confirmacion(){
        if(confirm('¿Esta seguro que desea mover este plan diario?')){
          return true;
        }else{
          return false;
        }
